...
def main():
    ...
    if cfg.windowed_mode:
        tf.show()
        # Could modify the following dict if wanted to run differently after the second invoke of tf.show(). I used to
        # use this to pass different parameters to the functions using the "args" key in the dict, but now I've
        # converted all those parameters to config vars. So now, if you wanted to change things up mid-simulation in
        # this way, you'd have to do something different. Might have to bring those args back, or set a global flag?
        # But windowed_mode hasn't been in my work flow for a long time, so, not bothering to provide for this.
        events.execute_repeatedly(tasks=[
                {"invoke": vx.save_screenshot_repeatedly},
                {"invoke": plot.show_graphs},
                {"invoke": mt.apply_even_tangent_forces},
        ...
        def sim_finished() -> bool:
            ...
            # setup.equilibration_time accounts for equilibration; many time units (hundreds) but fast executing
            # (minutes) plus a few more for the sim proper if desired (slow; ~30 per hour on my old Intel Mac)
        ...
        def recoil_test(remodel_bonds: bool, duration: float) -> None:
            ...
            """
            # ToDo?
            # Might want to do something with plotting, here, to capture plots at the moment we switch mode. It's a
            # bit tricky, though, because of accumulating data for time averaging, so I haven't tried to implement.
