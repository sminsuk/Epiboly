...
# adjusting time_avg_accumulation_steps to compensate, which is defined in terms of timesteps (it counts dt intervals).
# (Other similar values – see plot_interval, sim_state_export_timestep_interval, screenshot_export_interval –
# are config'ed as units of Universe.time and then automatically converted to timesteps, so that you can adjust dt
# without having to make compensating adjustments in those. But I wasn't sure what exactly is the best behavior
# for time_avg_accumulation_steps, so I'm leaving it alone for now. May need to manually adjust as appopriate.)
dt: float = 0.1
...
sim_state_export_time_unit_interval: float = 10
sim_state_export_timestep_interval: int = round(sim_state_export_time_unit_interval / dt)
...
screenshot_export_interval_in_time_units: float = 2
screenshot_export_interval: int = max(1, round(screenshot_export_interval_in_time_units / dt))
...
# (Set the value in time units; calculated value in timesteps will be used during execution.)
plot_interval_in_time_units: float = 300
plot_interval: int = round(plot_interval_in_time_units / dt)
...
time_avg_accumulation_steps: int = min(plot_interval - 1, config_time_avg_accumulation_steps)
...
recoil_duration_without_remodeling: float = 100
recoil_duration_with_remodeling: float = 100
...
def get_state() -> dict:
    ...
    return {"config_values": {
                ...
                "sim_state_export_time_unit_interval": sim_state_export_time_unit_interval,
                "sim_state_export_minutes_interval": sim_state_export_minutes_interval,
                "sim_state_export_keep": sim_state_export_keep,
                "screenshot_export_interval_in_time_units": screenshot_export_interval_in_time_units,
                ...
                "plot_interval_in_time_units": plot_interval_in_time_units,
