...
This file temporarily duplicated from main, just to clean up certain lines for screenshots for my talk.
...
def initialize_interior(leading_edge_phi):
    ...
    if True:
        ...
        vectors = su.random_nd_spherical(npoints=2050, dim=3)
        edge_margin = 0.15
    else:
        ...
        vectors = tf.random_points(tf.PointsType.Sphere.value, 2050)
...
def initialize_bonded_edge():
    def create_ring():
        ...
        leading_edge_phi = su.phi_for_epiboly(epiboly_percentage=40)
        ...
        # Number of points = (circumference of latitude circle at this location) / r0
        # (Um, or maybe not? That looked like too much.)
        #         circ_latitude = 2 * math.pi * r_latitude
        #         num_points = round(circ_latitude / r0)
        
        # Just eyeball it for now. 25 was for debugging. After return to non-overlapping particles,
        # around 100 might be about right.
        num_points = 55
        ...
        unit_circle_vectors = su.random_nd_spherical(npoints=num_points, dim=2)
        
        # make 3D and scaled
        z_latitude = math.cos(leading_edge_phi) * scale
        latitude_center_position = big_particle.position + su.vec([0, 0, z_latitude])
        final_position = lambda unit_vector: (latitude_center_position
    ...
    def create_bonds():
        ...
        for particle in sorted_particles:
            ...
            tf.Bond.create(small_small_attraction, previous_particle, particle)
...
tf.bind.types(small_small_repulsion, Little, Little)
...
# small_small_attraction = tf.Potential.harmonic(r0 = r0, 
#                                                k = 0.05, 
#                                                min = r0, 
#                                                max = r0 + 0.04)
...
def plots():
    ...
    small_small_attraction.plot(potential=True, force=True, ymin=-0.001, ymax=0.01, min=0.28, max=0.34)
    # This fails ("Maximum allowed size exceeded"):
    # combo = small_small_repulsion + small_small_attraction
